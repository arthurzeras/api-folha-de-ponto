...
test('Tests for registers', async (t) => {
  await t.test('Should save a new entrance for /batidas endpoint', async () => {
  ...
  await t.test(
    'Should return a Bad Request error if not receive "momento" parameter',
    async () => {
  ...
  await t.test(
    'Should return a Bad Request error if "momento" parameter is not a valid date',
    async () => {
      const response = await request(app)
        .post('/batidas')
